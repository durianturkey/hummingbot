...
class CoinstoreAPIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    async def _request_order_book_snapshot(self, trading_pair: str) -> Dict[str, Any]:
        ...
        url = CONSTANTS.SNAPSHOT_PATH_URL + "/" + symbol
        rest_assistant = await self._api_factory.get_rest_assistant()
        data = await rest_assistant.execute_request(
            url=web_utils.public_rest_url(path_url=url, domain=self._domain),
    ...
    async def _subscribe_channels(self, ws: WSAssistant):
        ...
        try:
            ...
            for trading_pair in self._trading_pairs:
                symbol = await self._connector.exchange_symbol_associated_to_pair(trading_pair=trading_pair)
                symbol_id = self._connector._get_symbol_id(symbol)
                trade_params.append(f"{symbol_id}@trade")
                depth_params.append(f"{symbol_id}@depth@100")
            payload = {"op": "SUB", "channel": trade_params, "param": {"size": 2}, "id": 1}
